...
export default function JobRequirements3D({
  ...
  // Create 3D nodes from the actual AI-analyzed job requirements
  const REQ_NODES = requirements.map((req, index) => ({
    id: req.name,
    label: req.name,
    category: req.category || "Requirement",
    path: `AI Job Analysis → ${req.category || "Requirement"}`,
    status: "validated",
    radius: 2.2 + (index % 3) * 0.2,
    angle:
      (Math.PI * 2 * index) /
  ...
  useEffect(() => {
    ...
    const centerGeo = new THREE.OctahedronGeometry(
      0.85,
      2
    );
    ...
    // Inner core
    const innerCoreGeo = new THREE.SphereGeometry(
      0.5,
      24,
      24
    );
    ...
    const animate = () => {
      ...
      REQ_NODES.forEach(
        (node, idx) => {
          ...
          lines[idx].geometry.setFromPoints(
            [
              new THREE.Vector3(
                0,
                0,
  ...
  return (
    <div
      ref={containerRef}
      className="relative w-full h-[400px] md:h-[480px] overflow-hidden select-none"
    >
      <canvas
        ref={canvasRef}
